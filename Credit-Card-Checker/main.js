...
function validateCred(arr) {
  ...
  for (let i = arr.length - 1; i >= 0; i--) {
    if ((arr.length - 1 - i) % 2 === 0) {
      ...
      let double = arr[i] * 2;
      if (double > 9) {
        secondArr.push(double - 9);
      } else {
        secondArr.push(double);
      }
    }
  }

  const sum = secondArr.reduce((accumulator, currentValue) => {
...
function idInvalidCompanies(cards) {
  let companies = [];
  cards.forEach((card) => {
  ...
  console.log('Invalid Company cards: ')
  return companies;
}
